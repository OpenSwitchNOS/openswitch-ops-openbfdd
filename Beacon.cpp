 ...
 */
static void
beacon_signal_handler(int sig)
{
  if (sig == SIGTERM)
  {
	g_beacon->RequestShutdown();
  }
}
#endif
...
bool Beacon::Run(const list<SockAddr> &controlPorts, const list<IpAddr> &listenAddrs)
{
...
#ifdef ENABLE_OVSDB
  // OVSDB command processor thread
  {
    SockAddr unused_addr;
    CommandProcessor *ovsProcessor = MakeOvsCommandProcessor(m_scheduler, this);
    commandProcessors->push_back(ovsProcessor);
    if (!ovsProcessor->BeginListening(unused_addr))
    {
      gLog.LogError("Failed to start OVS command processing thread on. Aborting.");
      return false;
    }
  }
  ...
  signal(SIGTERM, beacon_signal_handler);
#endif
