...
bfd_session_diag_backend_enum_to_ovsdb_string(int diag_backend_value)
{
	...
	switch (diag_backend_value) {
		case BFD_OVSDB_IF_SESSION_DIAG_NONE:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_NONE;
		case BFD_OVSDB_IF_SESSION_DIAG_CONTROL_DETECT_EXPIRED:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_CONTROL_DETECT_EXPIRED;
		case BFD_OVSDB_IF_SESSION_DIAG_ECHO_FAILED:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_ECHO_FAILED;
		case BFD_OVSDB_IF_SESSION_DIAG_NEIGHBOR_SESSION_DOWN:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_NEIGHBOR_SESSION_DOWN;
		case BFD_OVSDB_IF_SESSION_DIAG_FORWARDING_RESET:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_FORWARDING_RESET;
		case BFD_OVSDB_IF_SESSION_DIAG_PATH_DOWN:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_PATH_DOWN;
		case BFD_OVSDB_IF_SESSION_DIAG_CONCAT_PATH_DOWN:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_REVERSE_CONCAT_PATH_DOWN;
		case BFD_OVSDB_IF_SESSION_DIAG_ADMIN_DOWN:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_ADMIN_DOWN;
		case BFD_OVSDB_IF_SESSION_DIAG_REVERSE_CONCAT_PATH_DOWN:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_CONCAT_PATH_DOWN;
		case BFD_OVSDB_IF_SESSION_DIAG_MAX:
			return OVSREC_BFD_SESSION_DIAGNOSTIC_MAX_DIAGNOSTICS;
		default:
			return "Unknown";
	}
}
...
bfd_apply_global_changes(struct ovsdb_idl *idl)
{
	...
	if (!ovs_bfd) {
		VLOG_WARN("BFD: Failed to get first BFD table\n");
		return false;
	}

	memset(&bfd_if_global, 0, sizeof(bfdOvsdbIfGlobal_t));

	if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_bfd_col_enable, idl_seqno)) {
	}
	if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_bfd_col_min_rx, idl_seqno)) {
		bfd_if_global.minRxInterval = *ovs_bfd->min_rx;
		SET_VALID(bfd_if_global.valid, BFD_OVSDB_IF_GLOBAL_MIN_RX_INTERVAL);
		is_changed = true;
	}
	if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_bfd_col_min_tx, idl_seqno)) {
		bfd_if_global.minTxInterval = *ovs_bfd->min_tx;
		SET_VALID(bfd_if_global.valid, BFD_OVSDB_IF_GLOBAL_MIN_TX_INTERVAL);
		is_changed = true;
	}
	if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_bfd_col_decay_min_rx, idl_seqno)) {
		bfd_if_global.multiplier = (uint8_t)*ovs_bfd->decay_min_rx;
	...
	if (is_changed) {
		...
		return bfdBackendSetGlobals(bfd_ovsdb_global.beacon, &bfd_if_global);
	}
...
static void
delete_bfd_sessions(struct ovsdb_idl *idl)
{
	const struct ovsrec_bfd_session *ovs_bfd_sess;
	bfdOvsdbIfSession_t bfd_if_session;

	OVSREC_BFD_SESSION_FOR_EACH(ovs_bfd_sess, idl) {
		if (ovsrec_bfd_session_row_get_seqno(ovs_bfd_sess, OVSDB_IDL_CHANGE_DELETE) > 0) {
			...
			if (!bfdBackendHandleSession(bfd_ovsdb_global.beacon, &bfd_if_session)) {
...
static void
bfd_session_read_ovsdb_apply_changes(struct ovsdb_idl *idl)
{
	const struct ovsrec_bfd_session *ovs_bfd_sess;
	bfdOvsdbIfSession_t bfd_if_session;

	OVSREC_BFD_SESSION_FOR_EACH(ovs_bfd_sess, idl) {
		if (!OVSREC_IDL_IS_ROW_INSERTED(ovs_bfd_sess, idl_seqno) &&
			!OVSREC_IDL_IS_ROW_MODIFIED(ovs_bfd_sess, idl_seqno)) {
                    continue;
		}

		memset(&bfd_if_session, 0, sizeof(bfdOvsdbIfSession_t));

		if (NEW_ROW(ovs_bfd_sess, idl_seqno)) {
			bfd_if_session.remote_address = ovs_bfd_sess->bfd_dst_ip;
			bfd_if_session.local_address = ovs_bfd_sess->bfd_src_ip;
		...
		// More changes like modification and confirmed transaction changes? - Later!
	}
...
void
bfd_ovsdb_init_context(void *beacon)
{
	bfd_ovsdb_global.beacon = beacon;
	return;
}
