...
static int
cli_bfd_session_cmd_execute(const char *remote_str, const char *local_str, char *from, int64_t asn)
{
	const struct ovsrec_bfd_session *ovs_bfd_session;
...
DEFUN(bfd_session,
      bfd_session_cmd,
      "bfd session remote (A.B.C.D|X:X::X:X|WORD) local (A.B.C.D|X:X::X:X|WORD) from WORD asn <1-4294967295>",
      "BFD specific commands"
      "Session\n"
      "Remote\n"
      "Remote address\nRemote IPv6 address\nWord\n"
      ...
      "Local address\nLocal IPv6 address\nWord\n"
      "Owner\n"
      "Session from - cli, bgp, other\n"
      "BGP AS-Number\n"
      "AS-Number\n")
#else
DEFUN(bfd_session,
      bfd_session_cmd,
      "bfd session remote A.B.C.D local A.B.C.D from WORD asn <1-4294967295>",
      "BFD specific commands"
      "Session\n"
      "Remote\n"
      "Remote address\n"
...
#endif
{
	...
	cli_bfd_session_cmd_execute(argv[0], argv[1], from, asn);
	return CMD_SUCCESS;
}
...
void cli_pre_init(void)
{
	...
	retval = install_show_run_config_context(e_vtysh_config_context,
			&vtysh_bfd_context_clientcallback,
			NULL, NULL);
