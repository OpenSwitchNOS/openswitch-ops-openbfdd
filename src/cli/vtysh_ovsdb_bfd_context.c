...
/* Context callback */
vtysh_ret_val
vtysh_bfd_context_clientcallback(void *p_private)
{
	...
	vtysh_ovsdb_cli_print(p_msg, "Fetching BFD config \n");

	system_row = ovsrec_system_first(p_msg->idl);
	if(system_row) {
		vtysh_ovsdb_cli_print(p_msg, "Got System config \n");
		if (system_row->n_bfd_enable > 0) {
		vtysh_ovsdb_cli_print(p_msg, "Inside BFD Enable \n");
			vtysh_ovsdb_cli_print(p_msg, "bfd enable");
		} else {
		vtysh_ovsdb_cli_print(p_msg, "Inside else BFD Enable \n");
		}
		...
		vtysh_ovsdb_cli_print(p_msg, "Inside BFD Interval \n");
			vtysh_ovsdb_cli_print(p_msg, "bfd interval %d min_rx %d multiplier %d",
					interval, min_rx, decay_min_rx);
		} else {
		vtysh_ovsdb_cli_print(p_msg, "Inside else BFD Interval \n");
		}
	}
...
vtysh_bfd_context_init(void *p_private)
{
}

void
vtysh_bfd_context_exit(struct feature_sorted_list * row_list)
{
}
#endif
